<template>
  ...
  >
    ...
    <div v-show="!isMinimized">
      ...
      <div class="tip" :dir="$i18n.locale === 'he' ? 'rtl' : 'ltr'">
        {{ $t(tipKey) }}
      </div>
      ...
      <div class="expanded">
        ...
        <div class="step">
          ...
          <!-- SUBTRACTION LAYOUT -->
          <template v-else>
            <div class="fruit-subtraction-group flex gap-1" dir="ltr">
              <div
                v-for="i in num1"
                :key="`n1-${i}`"
...
const onDrag = (e: MouseEvent | TouchEvent) => {
  ...
  if (
    Math.abs(newX - position.value.x) > 2 ||
    Math.abs(newY - position.value.y) > 2
  ) {
    hasMoved.value = true;
  }

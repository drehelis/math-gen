      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6" dir="ltr">
        ...
        >
          ...
          <div class="flex items-center justify-center pt-2" style="font-family: 'Space Mono', monospace;" dir="ltr">
            <span class="relative text-base sm:text-lg md:text-xl lg:text-2xl font-bold whitespace-nowrap" style="color: var(--color-deep);">
              <template v-if="question.missingPosition === 'first'">
                <AnswerInput v-if="!showAnswers" :correct-answer="question.answer" :hide-feedback="true" @feedback="(data) => handleFeedback(question.id, data)" />
              ...
              <span
                ...
                :style="{
                  color: feedbackState[question.id]?.isCorrect ? '#15803d' : '#b91c1c',
                  fontSize: '2rem',
                  right: '-2rem',
                  top: '50%',
              ...
              >
                {{ feedbackState[question.id]?.show ? (feedbackState[question.id]?.isCorrect ? '✔' : '✖') : '' }}
              </span>
            </span>
          </div>
        </div>

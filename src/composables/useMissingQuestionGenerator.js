...
export function useMissingQuestionGenerator() {
  ...
  if (savedSettings) {
    const validOps = (savedSettings.operations || []).filter(
  ...
  const generateStandardQuestion = (operation) => {
    if (operation === "addition") {
      ...
      const num1 = getRandomNumber(
        settings.value.varySecondNumber && varyFirst,
      );
      ...
      if (missingPosition === "first") {
        ...
        const num1 =
          result +
          getRandomNumber(settings.value.varySecondNumber && !varyFirst);
